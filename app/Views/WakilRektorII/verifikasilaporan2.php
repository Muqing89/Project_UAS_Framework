...
<body id="page-top">
                                    ...
                                    <tbody>
                                        <?php foreach ($laporan as $row): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modalDetail<?= $row['id'] ?>">Lihat Detail</button>
                                                </td>
                                                <td>
    ...
    <?php foreach ($laporan as $row): ?>
        <div class="modal fade" id="modalDetail<?= $row['id'] ?>" tabindex="-1" role="dialog">
            <div class="modal-dialog modal-lg">
                <form action="<?= base_url('verifikasilaporanWR/' . $row['id']) ?>" method="post">
                    <div class="modal-content">
                        <div class="modal-header bg-primary text-white">
                            <h5 class="modal-title">Detail Laporan UKM</h5>
                        ...
                        <div class="modal-body">
                            ...
                            <p><strong>Dokumen:</strong>
                                <a href="<?= base_url('uploads/laporan/' . $row['dokumen_laporan']) ?>" target="_blank">Lihat Dokumen</a>
                            </p>
                            <p><strong>Status WR II:</strong>
                                <?= !empty($row['status_wr']) ? esc($row['status_wr']) : 'Belum Diproses' ?>
                            </p>
                        </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <?php else : ?>
                                <button class="btn btn-secondary" disabled>Laporan belum diverifikasi Admin Kemahasiswaan</button>
                            <?php endif; ?>
    ...
    <!-- Modal Komen -->
    <?php foreach ($laporan as $row): ?>
        <div class="modal fade" id="modalKomenWR<?= $row['id'] ?>" tabindex="-1" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-warning text-white">
                        <h5 class="modal-title">Komentar WR II</h5>
                        <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
                    </div>
                    <form action="<?= base_url('simpanKomenLaporanWR/' . $row['id']) ?>" method="post">
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="komentar_wr">Tulis Komentar:</label>
                                <textarea name="komentar_wr" class="form-control" rows="4" required><?= esc($row['komentar_wr'] ?? '') ?></textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-warning text-white">Simpan Komentar</button>

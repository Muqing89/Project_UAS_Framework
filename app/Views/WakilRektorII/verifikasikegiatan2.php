...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Data Table untuk Verifikasi Kegiatan oleh Wakil Rektor II -->
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Verifikasi Kegiatan UKM - WR II</h6>
                        </div>
                        <div class="card-body">
    ...
    <!-- Modal Detail Kegiatan Wakil Rektor II -->
    <?php foreach ($kegiatan as $row): ?>
        <div class="modal fade" id="modalWRDetail<?= $row['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="modalWRLabel<?= $row['id'] ?>" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <form action="<?= base_url('verifikasiKegiatanWR/' . $row['id']) ?>" method="post">
                    <?= csrf_field() ?>
                    <div class="modal-content">
                        <div class="modal-header bg-primary text-white">
                            <h5 class="modal-title" id="modalWRLabel<?= $row['id'] ?>">Detail Kegiatan - Wakil Rektor II</h5>
                        ...
                        <div class="modal-body text-left">
                            ...
                            <p><strong>Ketua Pelaksana:</strong> <?= esc($row['ketua_pelaksana']) ?></p>
                            <p><strong>Tanggal Kegiatan:</strong> <?= esc($row['tanggal_kegiatan']) ?></p>
                            <p><strong>Waktu Kegiatan:</strong> <?= esc($row['waktu_kegiatan']) ?></p>
                            <p><strong>Tempat Kegiatan:</strong> <?= esc($row['tempat_kegiatan']) ?></p>
                            <p><strong>Jumlah Peserta:</strong> <?= esc($row['jumlah_peserta']) ?></p>
                            <p><strong>Target Peserta:</strong> <?= esc($row['target_peserta']) ?></p>
                            <p><strong>Deskripsi:</strong><br><?= esc($row['deskripsi']) ?></p>
                            <p><strong>Status WR II:</strong> <?= esc($row['status_wr']) ?></p>
                        </div>

                        <div class="modal-footer">
                            <?php if ($row['status_wr'] === 'Disetujui WR II'): ?>
                                <button class="btn btn-secondary" disabled>Kegiatan sudah diverifikasi</button>
                            <?php elseif ($row['status_admin'] === 'Terverifikasi' || $row['status_wr'] === 'Ditolak') : ?>
                                <button type="submit" name="aksi" value="verifikasi" class="btn btn-success" onclick="return confirm('Yakin ingin memverifikasi kegiatan ini?')">Verifikasi</button>
                                <button type="submit" name="aksi" value="tolak" class="btn btn-danger" onclick="return confirm('Yakin ingin menolak kegiatan ini?')">Tolak</button>
                                <button type="button" class="btn btn-warning text-white" data-toggle="modal" data-target="#modalKomenWR<?= $row['id'] ?>" data-dismiss="modal">Komentar</button>
                            <?php else : ?>
                                <button class="btn btn-secondary" disabled>Kegiatan belum diverifikasi Admin Kemahasiswaan</button>
                            <?php endif; ?>
    ...
    <!-- Modal Komentar Wakil Rektor II -->
    <?php foreach ($kegiatan as $row): ?>
        <div class="modal fade" id="modalKomenWR<?= $row['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="modalKomenWRLabel<?= $row['id'] ?>" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <form action="<?= base_url('simpanKomenKegiatanWR/' . $row['id']) ?>" method="post">
                    <?= csrf_field() ?>
                    <div class="modal-content">
                        <div class="modal-header bg-primary text-white">
                            <h5 class="modal-title" id="modalKomenWRLabel<?= $row['id'] ?>">Tulis Komentar - Wakil Rektor II</h5>
                            <button type="button" class="close text-white" data-dismiss="modal" aria-label="Tutup">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
